...
`codegen/SDK_CODEGEN.md` documents the compiler boundary and drift rules. `codegen/SDK_SEMANTICS.md` documents the Mistral semantic policy. Preserve the vendored official spec exactly; preprocessing operates on a temporary generation copy and fails closed when its expected source shape changes.
