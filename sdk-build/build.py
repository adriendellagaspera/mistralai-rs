"""Build and verify the Mistral SDK using pinned standalone Rust tools."""
...
def require_sdk_parity(
    statuses: Counter[str],
    facade_delta: list[str],
) -> None:
    """Reject incomplete operation coverage or changes to the published SDK facade."""
    if statuses.get("rejected", 0):
        raise ValueError(
            f"SDK coverage gate: {statuses['rejected']} OpenAPI operations remain rejected"
        )
    if facade_delta:
        raise ValueError(
            "SDK compatibility gate: generated SDK facade differs from the "
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=".sdk-build-probe-", dir=ROOT) as temporary:
        ...
        runtime = {
            ...
            # Frozen historical marker is part of the byte-for-byte facade baseline.
        ...
        if require_parity or publish:
            require_sdk_parity(statuses, facade_delta)
        if publish:
