...
pub(crate) fn field<'a>(value: &'a Value, name: &str) -> Result<&'a Value> {
    value.get(name).ok_or_else(|| std::io::Error::other(format!("missing field {name}")).into())
}

pub(crate) fn string<'a>(value: &'a Value, name: &str) -> Result<&'a str> {
    field(value, name)?.as_str().ok_or_else(|| std::io::Error::other(format!("{name} is not a string")).into())
}

fn source_operations(spec: &Value) -> Result<BTreeMap<String, Value>> {
    let mut source = BTreeMap::new();
    let paths = field(spec, "paths")?.as_object().ok_or("OpenAPI paths must be an object")?;
    for (path, item) in paths {
        let Some(methods) = item.as_object() else { continue };
        for (method, operation) in methods {
            if !matches!(method.to_ascii_lowercase().as_str(), "get" | "put" | "post" | "delete" | "patch" | "head" | "options" | "trace" | "query") {
                continue;
            }
            let Some(op) = operation.as_object() else { continue };
            let Some(id) = op.get("operationId").and_then(Value::as_str).filter(|id| !id.is_empty()) else { continue };
            let mut media = BTreeSet::new();
            if let Some(responses) = op.get("responses").and_then(Value::as_object) {
                for (status, response) in responses {
                    if !status.starts_with('2') { continue; }
                    if let Some(content) = response.get("content").and_then(Value::as_object) {
                        media.extend(content.keys().cloned());
                    }
                }
            }
            if source.insert(id.into(), json!({
...
pub(crate) fn source_paths(spec: &Value) -> Result<BTreeMap<String, String>> {
    ...
    for (path, methods) in paths {
        ...
        for (method, operation) in methods {
            ...
            let Some(id) = operation.get("operationId").and_then(Value::as_str).filter(|id| !id.is_empty()) else { continue };
            if let Some(previous) = result.insert(id.into(), path.clone()) {
                if previous != *path {
                    return fail(format!("OpenAPI operationId {id} appears at both {previous} and {path}"));
                }
            }
        }
    }
...
pub(crate) fn raw_coverage(generated: &Path, spec: &Value) -> Result<Value> {
    ...
    let operations = field(&manifest, "operations")?.as_array().ok_or("binding operations must be an array")?;
    let mut emitted = BTreeSet::new();
    for binding in operations {
        if binding.get("kind").and_then(Value::as_str) != Some("call_shape") { continue; }
        ...
        let Some(expected) = source.get(id) else { return fail(format!("raw binding operation missing from OpenAPI: {id}")) };
        if expected["method"] != method || expected["path"] != path {
            return fail(format!("raw binding operation disagrees with OpenAPI: {id}"));
        }
        emitted.insert(id.to_owned());
    }
    let expected: BTreeSet<_> = source.keys().cloned().collect();
    if emitted != expected {
        return fail(format!("raw binding coverage disagrees with OpenAPI: missing={:?}; extra={:?}",
...
fn identities(coverage: &Value) -> Result<BTreeMap<String, (String, String)>> {
    let mut result = BTreeMap::new();
    let operations = field(coverage, "operations")?.as_array().ok_or("raw coverage operations must be an array")?;
    for operation in operations {
        let id = string(operation, "operation_id")?;
        if result.insert(id.into(), (string(operation, "method")?.into(), string(operation, "path")?.into())).is_some() {
...
fn normalized_manifest(data: &[u8], operation_paths: &BTreeMap<String, String>) -> Result<Value> {
    ...
    if let Some(operations) = value.get_mut("operations").and_then(Value::as_array_mut) {
        for operation in operations {
            let Some(origin) = operation.get_mut("source_operation").and_then(Value::as_object_mut) else { continue };
            let id = origin.get("operation_id").and_then(Value::as_str).ok_or("manifest operation missing operation_id")?;
            let Some(path) = operation_paths.get(id) else { return fail(format!("manifest operation {id} missing from OpenAPI")) };
...
pub(crate) fn verify_raw_baseline(generated: &Path, committed: &Path, spec: &Value) -> Result<()> {
    ...
    if actual.keys().collect::<Vec<_>>() != expected.keys().collect::<Vec<_>>() {
        return fail(format!("raw file inventory drifted: expected={:?}; actual={:?}", expected.keys().collect::<Vec<_>>(), actual.keys().collect::<Vec<_>>()));
    }
    ...
    for (name, after) in actual {
        ...
        if name == "binding-manifest.json" {
            ...
            let normalized = String::from_utf8_lossy(before).replace("../sources/openapi/openapi.yaml", "openapi/published.yaml");
            if normalized.as_bytes() != after {
                return fail(format!("raw output drifted beyond source provenance marker: {name}"));
            }
        }
    }
    Ok(())
...
pub(crate) fn verify_overlaid(spec: &Value) -> Result<()> {
    ...
    let required = |value: &Value, needle: &str| {
        value.get("required").and_then(Value::as_array).is_some_and(|items| items.iter().any(|item| item.as_str() == Some(needle)))
    };
    ...
    let paths = field(spec, "paths")?.as_object().ok_or("OpenAPI paths must be an object")?;
    let actual: BTreeSet<_> = paths.keys().filter(|p| p.contains("#stream") || p.contains("#wav")).map(String::as_str).collect();
    ...
    if actual != expected {
        return fail(format!("overlay changed published representation-specific path inventory: {actual:?}"));
    }
    Ok(())
}

pub(crate) fn validate_coverage(report: &Value, baseline: &Value, spec: &Value) -> Result<BTreeMap<String, usize>> {
    if baseline["schema_version"] != 1 { return fail("unsupported SDK coverage baseline schema") }
    let sources: BTreeSet<_> = source_paths(spec)?.into_keys().collect();
    let operations = field(report, "operations")?.as_object().ok_or("SDK derivation report missing operations")?;
    let actual: BTreeSet<_> = operations.keys().cloned().collect();
    if sources.len() != baseline["total_operations"].as_u64().unwrap_or(0) as usize || sources != actual {
        return fail(format!("SDK operation inventory drift: source={}, report={}, expected={}; missing={:?}; extra={:?}",
            sources.len(), actual.len(), baseline["total_operations"],
            sources.difference(&actual).collect::<Vec<_>>(),
            actual.difference(&sources).collect::<Vec<_>>()));
    }
    let list = |name: &str| -> Result<Vec<String>> {
        Ok(field(baseline, name)?.as_array().ok_or("coverage baseline list must be an array")?
            .iter().map(|v| v.as_str().map(str::to_owned).ok_or("coverage identity must be a string"))
    ...
    if rejected.len() != allowed_rejected.len() || overridden.len() != allowed_overridden.len()
        || !allowed_rejected.is_disjoint(&allowed_overridden)
        || !allowed_rejected.union(&allowed_overridden).all(|id| sources.contains(id)) {
    ...
    for (id, item) in operations {
        ...
        *counts.entry(status.to_owned()).or_insert(0) += 1;
        if status == "rejected" && !allowed_rejected.contains(id) { new_rejections.push(id); }
        if status == "overridden" && !allowed_overridden.contains(id) { new_overrides.push(id); }
    }
    if !new_rejections.is_empty() || !new_overrides.is_empty() {
        return fail(format!("SDK coverage regression: new_rejections={new_rejections:?}; unreviewed_overrides={new_overrides:?}"));
    }
    Ok(counts)
}

pub(crate) fn require_publish_parity(counts: &BTreeMap<String, usize>, delta: &[String]) -> Result<()> {
    if counts.get("rejected").copied().unwrap_or(0) > 0 {
        return fail(format!("cannot publish SDK: {} operations remain rejected", counts["rejected"]));
    }
    if !delta.is_empty() {
        return fail(format!("cannot publish SDK: generated SDK facade differs from committed public baseline in {} files: {}", delta.len(), delta.join(", ")));
    }
    Ok(())
}
...
pub(crate) fn committed_facade(path: &Path) -> Result<BTreeMap<String, Vec<u8>>> {
    Ok(snapshot(path)?.into_iter().filter(|(name, _)| name.ends_with(".rs") && name != "error.rs").collect())
}
...
pub(crate) fn copy_dir(source: &Path, target: &Path) -> Result<()> {
    ...
    for entry in fs::read_dir(source)? {
        ...
        else if path.is_file() { fs::copy(path, dest)?; }
    }
    Ok(())
}
...
mod tests {
    ...
    fn rejects_missing_operation_and_unreviewed_override() {
        let spec = json!({"paths":{"/x":{"get":{"operationId":"a"}}, "/y":{"post":{"operationId":"b"}}}});
