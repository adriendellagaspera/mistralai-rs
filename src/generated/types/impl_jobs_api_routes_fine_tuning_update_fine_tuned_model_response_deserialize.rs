impl<'de> serde::Deserialize<'de> for JobsApiRoutesFineTuningUpdateFineTunedModelResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let value = serde_json::Value::deserialize(deserializer)?;
        let discriminator = match value.get("model_type") {
            Some(serde_json::Value::String(discriminator)) => Some(discriminator.as_str()),
            Some(_) => {
                return Err(serde::de::Error::custom(concat!(
                    "non-string discriminator `",
                    "model_type",
                    "`",
                )));
            }
            None => None,
        };
        match discriminator {
            Some(discriminator) => match discriminator {
                "completion" => {
                    let primary_error =
                        match serde_json::from_value::<CompletionFTModelOut>(value.clone()) {
                            Ok(payload) => return Ok(Self::CompletionFTModelOut(payload)),
                            Err(error) => error,
                        };
                    let mut structural_match: Option<(Self, &'static str)> = None;
                    if let Ok(payload) =
                        serde_json::from_value::<ClassifierFTModelOut>(value.clone())
                    {
                        if let Some((_, first_name)) = &structural_match {
                            return Err(serde::de::Error::custom(format!(
                                "discriminator `{}` value `{}` did not fit its mapped branch and structurally matched both `{}` and `{}`",
                                "model_type",
                                "completion",
                                first_name,
                                stringify!(ClassifierFTModelOut),
                            )));
                        }
                        structural_match = Some((
                            Self::ClassifierFTModelOut(payload),
                            stringify!(ClassifierFTModelOut),
                        ));
                    }
                    match structural_match {
                        Some((payload, _)) => Ok(payload),
                        None => Err(serde::de::Error::custom(primary_error)),
                    }
                }
                "classifier" => {
                    let primary_error =
                        match serde_json::from_value::<ClassifierFTModelOut>(value.clone()) {
                            Ok(payload) => return Ok(Self::ClassifierFTModelOut(payload)),
                            Err(error) => error,
                        };
                    let mut structural_match: Option<(Self, &'static str)> = None;
                    if let Ok(payload) =
                        serde_json::from_value::<CompletionFTModelOut>(value.clone())
                    {
                        if let Some((_, first_name)) = &structural_match {
                            return Err(serde::de::Error::custom(format!(
                                "discriminator `{}` value `{}` did not fit its mapped branch and structurally matched both `{}` and `{}`",
                                "model_type",
                                "classifier",
                                first_name,
                                stringify!(CompletionFTModelOut),
                            )));
                        }
                        structural_match = Some((
                            Self::CompletionFTModelOut(payload),
                            stringify!(CompletionFTModelOut),
                        ));
                    }
                    match structural_match {
                        Some((payload, _)) => Ok(payload),
                        None => Err(serde::de::Error::custom(primary_error)),
                    }
                }
                other => Err(serde::de::Error::custom(format!(
                    "unknown discriminator value `{other}` for `{}`",
                    "model_type",
                ))),
            },
            None => {
                let mut structural_match: Option<(Self, &'static str)> = None;
                if let Ok(payload) = serde_json::from_value::<CompletionFTModelOut>(value.clone()) {
                    if let Some((_, first_name)) = &structural_match {
                        return Err(serde::de::Error::custom(format!(
                            "missing discriminator `{}` structurally matched both `{}` and `{}`",
                            "model_type",
                            first_name,
                            stringify!(CompletionFTModelOut),
                        )));
                    }
                    structural_match = Some((
                        Self::CompletionFTModelOut(payload),
                        stringify!(CompletionFTModelOut),
                    ));
                }
                if let Ok(payload) = serde_json::from_value::<ClassifierFTModelOut>(value.clone()) {
                    if let Some((_, first_name)) = &structural_match {
                        return Err(serde::de::Error::custom(format!(
                            "missing discriminator `{}` structurally matched both `{}` and `{}`",
                            "model_type",
                            first_name,
                            stringify!(ClassifierFTModelOut),
                        )));
                    }
                    structural_match = Some((
                        Self::ClassifierFTModelOut(payload),
                        stringify!(ClassifierFTModelOut),
                    ));
                }
                structural_match.map(|(payload, _)| payload).ok_or_else(|| {
                    serde::de::Error::custom(concat!(
                        "missing string discriminator `",
                        "model_type",
                        "` and no tagless branch matched",
                    ))
                })
            }
        }
    }
}
