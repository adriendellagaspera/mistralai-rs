...
use mistralai::{AudioTranscriptionRequest, ChatCompletionRequest, Client, CompletionChunk,
    EmbeddingRequest, FilesApiRoutesUploadFileRequest, streaming};
use serde_json::json;
...
async fn transcription_multipart_preserves_file_arrays_and_null_omission() {
    ...
    request.timestamp_granularities = Some(serde_json::from_value(json!(["word","segment"])).unwrap());
    request.language = Some(None);
    let response = Client::new().with_base_url(&url).with_api_key("test-key")
        .with_upload_filename("sample.wav")
        .audio_api_v1_transcriptions_post(request).await.unwrap();
    ...
    assert!(headers.to_lowercase().contains("content-type: multipart/form-data; boundary="));
    assert!(headers.to_lowercase().contains("authorization: bearer test-key"));
...
async fn file_upload_decodes_typed_response_and_sends_filename() {
    let body = json!({"id":"00000000-0000-0000-0000-000000000001", "object":"file",
        ...
        "purpose":"batch","sample_type":"batch_request","source":"upload"}).to_string();
    let (url, worker) = server("200 OK", "application/json", body.as_bytes());
    let request = FilesApiRoutesUploadFileRequest::new(Bytes::from_static(b"{}\n"));
    let response = Client::new().with_base_url(&url).with_upload_filename("batch.jsonl")
        .files_api_routes_upload_file(request).await.unwrap();
...
async fn binary_downloads_and_wav_use_correct_media_and_escaped_paths() {
    ...
    let result = Client::new().with_base_url(&url)
        .files_api_routes_download_file("a/b ?").await.unwrap();
    ...
    let result = Client::new().with_base_url(&url)
        .get_voice_sample_audio_v1_audio_voices_voice_id_sample_get_wav("test").await.unwrap();
    assert_eq!(result.as_ref(), bytes);
    let (headers, _) = worker.join().unwrap();
...
async fn embeddings_send_typed_json_and_expose_usage() {
    let body = json!({"id":"e1","object":"list","model":"test","data":[
        ...
        "usage":{"prompt_tokens":4,"completion_tokens":0,"total_tokens":4}}).to_string();
    let (url, worker) = server("200 OK", "application/json", body.as_bytes());
    let request: EmbeddingRequest = serde_json::from_value(json!({"model":"test","input":["hello"]})).unwrap();
    let result = Client::new().with_base_url(&url).embeddings_v1_embeddings_post(request).await.unwrap();
    assert_eq!(result.usage.prompt_tokens, 4);
    assert_eq!(result.data.len(), 1);
    let (headers, payload) = worker.join().unwrap();
    assert!(headers.starts_with("POST /v1/embeddings HTTP/1.1"));
    assert_eq!(serde_json::from_slice::<serde_json::Value>(&payload).unwrap()["input"], json!(["hello"]));
}
...
async fn generated_chat_stream_sends_stream_flag_and_decodes_sse() {
    ...
    let request: ChatCompletionRequest = serde_json::from_value(json!({"model":"test",
        "messages":[{"role":"user","content":"hello"}]})).unwrap();
    let client = Client::new().with_base_url(&url).with_api_key("test-key");
    let bytes = client.chat_completion_v1_chat_completions_post_stream(request).await.unwrap();
    let events: Vec<_> = streaming::json_events::<_, _, CompletionChunk>(bytes).collect().await;
    ...
    assert!(headers.to_lowercase().contains("authorization: bearer test-key"));
    assert_eq!(serde_json::from_slice::<serde_json::Value>(&payload).unwrap()["stream"], true);
}

#[test]
fn repaired_workflow_list_and_sharing_delete_match_wire_fields() {
    let workflows: mistralai::WorkflowListResponse = serde_json::from_value(json!({"workflows":[],"next_cursor":null})).unwrap();
    assert!(workflows.workflows.is_empty());
    assert!(serde_json::from_value::<mistralai::WorkflowListResponse>(json!({"next_cursor":null})).is_err());
    let sharing: mistralai::SharingDelete = serde_json::from_value(json!({
        "share_with_uuid":"00000000-0000-0000-0000-000000000001", "share_with_type":"user"})).unwrap();
    assert!(serde_json::to_value(sharing).unwrap().get("level").is_none());
}
...
async fn native_audio_stream_sets_mode_and_decodes_typed_envelope() {
    ...
    let bytes = client.audio_api_v1_transcriptions_post_stream(request).await.unwrap();
    let events: Vec<_> = streaming::events(bytes).collect().await;
    let event: mistralai::TranscriptionStreamEvents = events[0].as_ref().unwrap().envelope().unwrap();
...
async fn workflow_feed_sends_cursor_headers_without_buffering_json() {
    ...
    let client = Client::new().with_base_url(&url);
    let bytes = client.stream_v1_workflows_executions_execution_id_stream_get(
        "execution/id", Some("worker a"), Some("cursor-7")).await.unwrap();
    let events: Vec<_> = streaming::events(bytes).collect().await;
    assert_eq!(events.len(), 1);
    let (headers, _) = worker.join().unwrap();
    assert!(headers.starts_with("GET /v1/workflows/executions/execution%2Fid/stream?event_source=worker+a HTTP/1.1"));
