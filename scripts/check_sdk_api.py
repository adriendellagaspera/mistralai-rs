...
def main():
    ...
    parser.add_argument("--rustdoc", action="store_true", help="also run pinned compiler-aware semver checks")
    ...
    names = subprocess.check_output(["git", "ls-tree", "--name-only", base, "--", path], cwd=ROOT, text=True)
    if not names.strip():
        print("No previous facade API inventory: initial baseline requires PR review.")
        return
    before = json.loads(subprocess.check_output(["git", "show", f"{base}:{path}"], cwd=ROOT))
    after = json.loads((ROOT / path).read_text())
    report = compare_surface(before, after)
    print(json.dumps(report, indent=2))
    if report["classification"] == "review_required":
        raise SystemExit("Generated public API changed: explicit compatibility review required.")
    if args.rustdoc:
        version = json.loads((ROOT / "codegen.lock").read_text())["cargo_semver_checks_version"]
        install = ROOT / ".tools" / f"cargo-semver-checks-{version}"
        executable = install / "bin/cargo-semver-checks"
        if not executable.exists():
            subprocess.run(["cargo", "install", "cargo-semver-checks", "--version", version,
                            "--locked", "--root", str(install)], cwd=ROOT, check=True)
        ...
        subprocess.run([str(executable), "semver-checks", "--baseline-rev", base], cwd=ROOT, check=True)
